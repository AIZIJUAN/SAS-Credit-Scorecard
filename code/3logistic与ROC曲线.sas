...
/*读取所有_woe变量并储存到宏变量varname_woe中*/
proc sql noprint;
        select name into :varname_woe separated by ' ' 
                from dictionary.columns
                where libname=upcase("cc") and memname=upcase("cc_woe") 
                          and upcase(strip(name)) like "%_WOE";
         /*strip函数用来去除字符串前后空格*/
quit;
%put &varname_woe;
...
model bad_good (event='1')=&varname_woe / selection =stepwise  sls=0.05 sle=0.05;
OUTPUT OUT=cc.Pred_Probs    P=Pred_bad_good;
run;


/* Apply the macro */
%let DSin=cc.Pred_Probs;
%let ProbVar=Pred_bad_good;
%let DVVar=bad_good;
%let DSROC=DSROC;
%let cStat=;
